...
export default function RoomSetup({ profileName, member, initialCode = '', onJoined }: Props) {
  ...
  async function create() {
    ...
    catch (cause) { setError(cause instanceof Error ? cause.message : 'Could not open a group') }
  ...
  async function join() {
    ...
    catch (cause) { setError(cause instanceof Error ? cause.message : 'Could not join that group') }
    finally { setBusy(null) }
  }
  ...
  return (
    <div className="room-entry" aria-busy={Boolean(busy)}>
      <section className="room-entry__create">
        ...
        <button className="btn btn--primary btn--big" disabled={Boolean(busy)} onClick={() => void create()}>{busy === 'create' ? 'Opening group…' : 'Create group'}</button>
      </section>
      ...
      <form className="room-code-form" onSubmit={(event) => { event.preventDefault(); void join() }}>
        <label htmlFor="room-code">Group code</label>
