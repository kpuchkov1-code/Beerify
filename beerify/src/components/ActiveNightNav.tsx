...
export default function ActiveNightNav({ active, roomActive, nightMode, onChange }: Props) {
  ...
  const moreActive = (nightMode === 'group' && active === 'pubs') || active === 'stats' || active === 'history' || active === 'profile'
  ...
  return (
    <>
      <nav className="active-night-nav" aria-label="Active night navigation">
        ...
        {nightMode === 'group' ? <button className={active === 'crew' ? 'active-night-nav__item active-night-nav__item--active' : 'active-night-nav__item'} aria-current={active === 'crew' ? 'page' : undefined} onClick={() => onChange('crew')}><span className="active-night-nav__icon" aria-hidden="true">♟</span><span>Crew</span>{roomActive && <span className="app-nav__dot" aria-label="Room active" />}</button> : <button className={active === 'pubs' ? 'active-night-nav__item active-night-nav__item--active' : 'active-night-nav__item'} aria-current={active === 'pubs' ? 'page' : undefined} onClick={() => onChange('pubs')}><span className="active-night-nav__icon" aria-hidden="true">⌖</span><span>Pubs</span></button>}
      ...
      <div id={MORE_ID} className="active-night-more" popover="auto" aria-label="More destinations">
        {nightMode === 'group' && <button aria-current={active === 'pubs' ? 'page' : undefined} onClick={() => open('pubs')}><span aria-hidden="true">⌖</span><span><strong>Pubs</strong><small>Shared map, crawl, golf and bingo</small></span></button>}
