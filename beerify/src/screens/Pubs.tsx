...
export default function Pubs({ session, membership, draft, reducedMotion, onDraftChange, onCrawlChange, onOpenCrew }: Props) {
  ...
  if (view === 'golf') return <main className="screen pubs-screen pub-game-focus">
    <header className="page-header"><button className="icon-btn" aria-label="Back to crawl" onClick={() => setView('plan')}><AppIcon name="arrow-left" /></button><div><span className="page-kicker">PLAY THE CRAWL</span><h1>⛳ Pub Golf</h1><p>{golfStops.length} stops become tonight’s holes.</p></div></header>
  ...
  if (view === 'bingo') return <main className="screen pubs-screen pub-game-focus">
    <header className="page-header"><button className="icon-btn" aria-label="Back to crawl" onClick={() => setView('plan')}><AppIcon name="arrow-left" /></button><div><span className="page-kicker">PUB GAME</span><h1>▦ Pub Bingo</h1><p>The same seeded card, tracked live for the squad.</p></div></header>
  ...
  if (view === 'discover') return <main className="screen pubs-screen pubs-discover">
    <header className="page-header"><button className="icon-btn" aria-label="Back to crawl" onClick={() => setView('plan')}><AppIcon name="arrow-left" /></button><div><span className="page-kicker">ADD A STOP</span><h1>Find pubs</h1><p>{areaLabel || 'Search an area or use your location.'}</p></div></header>
  ...
  return <main className="screen pubs-screen pubs-plan">
    ...
    {crawl.length ? <>
      ...
      <section className="crawl-plan"><div className="section-heading"><h2>Stop order</h2><span>{crawl.length} / 12</span></div><ol className="crawl-list">{crawl.map((stop, index) => <li className={selectedId === stop.id ? 'crawl-stop crawl-stop--selected' : 'crawl-stop'} key={stop.id}><button className="crawl-stop__main" onClick={() => setSelectedId(stop.id)}><span>{index + 1}</span><span><strong>{stop.name}</strong><small>{stop.address || stop.type}</small></span></button><span className="crawl-stop__actions"><button disabled={index === 0 || (isGroup && (!membership?.isHost || live.game?.kind === 'pub-golf'))} aria-label={`Move ${stop.name} up`} onClick={() => moveStop(index, -1)}><AppIcon name="arrow-up" size={18} /></button><button disabled={index === crawl.length - 1 || (isGroup && (!membership?.isHost || live.game?.kind === 'pub-golf'))} aria-label={`Move ${stop.name} down`} onClick={() => moveStop(index, 1)}><AppIcon name="arrow-down" size={18} /></button><button disabled={isGroup && (!membership?.isHost || live.game?.kind === 'pub-golf')} aria-label={`Remove ${stop.name}`} onClick={() => void updateCrawl(crawl.filter((item) => item.id !== stop.id))}><AppIcon name="trash" size={18} /></button></span></li>)}</ol><button className="btn btn--primary btn--big" onClick={() => setView('discover')}>{isGroup && !membership?.isHost ? 'Browse nearby pubs' : 'Add pubs'}</button><div className="button-row"><a className="btn btn--secondary" href={mapsUrl} target="_blank" rel="noreferrer">Google route <AppIcon name="external" size={18} /></a><a className="btn btn--quiet" href={appleMapsUrl} target="_blank" rel="noreferrer">Apple Maps <AppIcon name="external" size={18} /></a></div></section>
    </> : <section className="pub-empty pub-empty--hero"><span aria-hidden="true"><AppIcon name="location" size={30} /></span><h2>Build tonight’s route</h2><p>Search ahead or start from where you are. You can add up to 12 stops.</p><div className="button-row"><button className="btn btn--primary" onClick={() => setView('discover')}>Search town or postcode</button><button className="btn btn--secondary" disabled={locating} onClick={requestLocation}>{locating ? 'Finding you…' : 'Use my location'}</button></div></section>}
