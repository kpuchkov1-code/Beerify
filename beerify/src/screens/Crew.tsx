...
export default function Crew({ profile, membership, session, initialCode, onJoin, onLeave, onOpenTonight }: Props) {
  if (!session) return (
    <main className="screen crew-screen">
      <header className="page-header page-header--stacked"><span className="page-kicker">START TOGETHER</span><h1>Crew starts with the night</h1><p>Choose Group in Tonight, then create or join once. The connection will stay fixed until your recap.</p></header>
  ...
  if (session.nightMode === 'solo') return (
    <main className="screen crew-screen">
      ...
      <p className="locked-room-copy">Solo or Group is chosen once so the rest of the night never changes underneath you.</p>
  ...
  return (
    <main className="screen crew-screen">
      ...
      {!membership && <p className="status-message" role="status">Your saved group connection expired. Rejoin with the same code, or create a replacement group; this night will stay in Group mode.</p>}
