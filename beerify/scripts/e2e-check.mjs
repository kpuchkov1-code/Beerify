...
try {
  ...
  await expect('Crew setup routes back through Tonight', await page.getByRole('heading', { name: 'Crew starts with the night' }).isVisible())
  ...
  const soloNav = page.getByRole('navigation', { name: 'Active night navigation' })
  await expect('solo navigation shows Drinks, Pubs, Games and More', await soloNav.getByRole('button').count() === 4 && await soloNav.getByRole('button', { name: 'Pubs', exact: true }).isVisible() && !(await soloNav.getByRole('button', { name: 'Crew', exact: true }).isVisible().catch(() => false)))
