...
try {
  ...
  await expect('before-night navigation is Tonight, Games, Pubs, Nights and You', await primaryNav.getByRole('button').allTextContents().then((labels) => labels.join('|') === 'Tonight|Games|Pubs|Nights|You'))
  ...
  for (const [viewport, width, height] of [['320', 320, 568], ['390', 390, 664], ['768', 768, 1024], ['1440', 1440, 900]]) {
    await page.setViewportSize({ width, height })
    for (const [tabName, screenName] of [['Tonight', 'tonight'], ['Games', 'games'], ['Pubs', 'pubs'], ['Nights', 'nights'], ['You', 'you']]) {
      await primaryNav.getByRole('button', { name: tabName, exact: true }).click()
      await primaryNav.getByRole('button', { name: tabName, exact: true }).waitFor()
      await page.waitForTimeout(100)
      await page.screenshot({ path: `${SHOTS}primary-${screenName}-${viewport}.png`, fullPage: true })
    }
  }
  await primaryNav.getByRole('button', { name: 'Tonight', exact: true }).click()
  await page.setViewportSize({ width: 320, height: 568 })
  await expect('favourites fit at 320px', await page.evaluate(() => document.documentElement.scrollWidth <= window.innerWidth))
  await expect('navigation labels do not truncate at 320px', await primaryNav.locator('.primary-nav__item').evaluateAll((items) => items.every((item) => item.scrollWidth <= item.clientWidth)))
  await expect('primary navigation targets are at least 44px', await primaryNav.locator('.primary-nav__item').evaluateAll((items) => items.every((item) => item.getBoundingClientRect().height >= 44)))
  ...
  for (const title of gameTitles) {
    ...
    if (title === gameTitles[0]) await expect('navigation hides during focused game play', !(await page.getByRole('navigation', { name: 'Primary navigation' }).isVisible()))
  ...
  await page.route('**/api/maps?*', async (route) => {
    ...
    if (url.searchParams.get('action') === 'nearby') return route.fulfill({ json: { venues: [
      { id: 'one', name: 'The Bottle Green', lat: 51.5, lng: -0.12, type: 'pub', distanceMeters: 40 },
      { id: 'two', name: 'The Hop Room', lat: 51.505, lng: -0.115, type: 'bar', distanceMeters: 720 },
      { id: 'three', name: 'Night Owl', lat: 51.51, lng: -0.11, type: 'nightclub', distanceMeters: 1450 },
    ] } })
  ...
  await expect('Pubs opens as a crawl-first planner', await page.getByRole('heading', { name: 'Build tonight’s route' }).isVisible())
  await page.getByRole('button', { name: 'Use my location' }).click()
  await page.getByText(/Location was not shared/).waitFor()
  await expect('location denial keeps town search usable', await page.getByText(/Search by town or postcode/).isVisible())
  await page.context().grantPermissions(['geolocation'], { origin: new URL(BASE).origin })
  await page.getByRole('button', { name: 'Use my location' }).click()
  await page.getByText('The Bottle Green').first().waitFor()
  await expect('discovery map has one permanent attribution control', await page.locator('.maplibregl-ctrl-attrib').count() === 1)
  ...
  await page.getByRole('button', { name: 'Back to crawl' }).click()
  await page.getByText(/1.8 km/).waitFor()
  await expect('draft crawl auto-routes and keeps all three stops', await page.getByText(/1.8 km/).isVisible() && await page.locator('.crawl-stop').count() === 3)
  await expect('Crew is absent before a night', !(await primaryNav.getByRole('button', { name: 'Crew', exact: true }).isVisible().catch(() => false)))
  await page.getByRole('button', { name: 'Tonight', exact: true }).click()
  await page.getByRole('button', { name: /Pissed/ }).click()
  await page.getByRole('button', { name: 'Start solo night' }).click()
  const soloNav = page.getByRole('navigation', { name: 'Primary navigation' })
  await expect('solo navigation is Drinks, Games, Pubs, Nights and You', await soloNav.getByRole('button').allTextContents().then((labels) => labels.join('|') === 'Drinks|Games|Pubs|Nights|You'))
  await soloNav.getByRole('button', { name: 'Pubs', exact: true }).click()
  await expect('saved draft moves into the Solo night', await page.locator('.crawl-stop').count() === 3)
  await page.getByRole('button', { name: /Pub Golf/ }).click()
  await expect('navigation hides during Pub Golf', !(await soloNav.isVisible()))
  ...
  await page.getByRole('button', { name: 'Back to crawl' }).click()
  await page.getByRole('button', { name: /Pub Bingo/ }).click()
  await expect('navigation hides during Pub Bingo', !(await soloNav.isVisible()))
  ...
  await page.getByRole('button', { name: 'Back to crawl' }).click()
  await soloNav.getByRole('button', { name: 'Drinks', exact: true }).click()
  await expect('meter starts at zero', await page.locator('.meter__value').getByText('.000', { exact: true }).isVisible())
  await soloNav.getByRole('button', { name: 'Nights', exact: true }).click()
  await page.getByRole('heading', { name: 'Your nights' }).waitFor()
  await expect('Nights opens without ending the Solo night', await page.getByRole('heading', { name: 'Your nights' }).isVisible())
  await soloNav.getByRole('button', { name: 'You', exact: true }).click()
  await page.getByRole('heading', { name: 'Your profile' }).waitFor()
  await expect('You opens without ending the night', await page.getByRole('heading', { name: 'Your profile' }).isVisible())
  await page.getByRole('switch', { name: 'Big-thumb mode' }).check()
  await page.getByRole('navigation', { name: 'Primary navigation' }).getByRole('button', { name: 'Drinks', exact: true }).click()
  ...
  await expect('navigation hides while a dialog is open', !(await page.getByRole('navigation', { name: 'Primary navigation' }).isVisible()))
  ...
  await expect('ending opens Your nights', await page.getByRole('heading', { name: 'Your nights' }).isVisible())
  await expect('Nights contains the newest session first', await page.locator('.nights-latest__receipt').getByText(/2 drinks/).isVisible())
  await page.locator('.nights-latest__receipt').click()
